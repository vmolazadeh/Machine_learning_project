...
This section imports the trained model 
"""

nn_state_dict = torch.load('modelnn.pt')
number_of_neurons=len(nn_state_dict['layer1.weight'][0])
loaded_model = MLPNN_Regression.Net(len(scaled_data[0]),number_of_neurons,1)
loaded_model.load_state_dict(nn_state_dict)
...
"""
scaled_prediction=loaded_model(torch.tensor(scaled_data.astype(np.float32)))
